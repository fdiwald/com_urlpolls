...
/**
 * Urlpolls component helper.
 */
abstract class UrlpollsHelper
{
	/**
	 * Composer Switch
	 * 
	...
	{
		$manifestUrl = JPATH_ADMINISTRATOR."/components/com_urlpolls/urlpolls.xml";
	...
	{
		// get params
		$params	= JComponentHelper::getParams('com_urlpolls');
	...
	public static function addSubmenu($submenu)
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_URLPOLLS_SUBMENU_DASHBOARD'), 'index.php?option=com_urlpolls&view=urlpolls', $submenu === 'urlpolls');
		if ($user->authorise('person.access', 'com_urlpolls') && $user->authorise('person.submenu', 'com_urlpolls'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_URLPOLLS_SUBMENU_PERSONS'), 'index.php?option=com_urlpolls&view=persons', $submenu === 'persons');
		}
		if ($user->authorise('poll.access', 'com_urlpolls') && $user->authorise('poll.submenu', 'com_urlpolls'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_URLPOLLS_SUBMENU_POLLS'), 'index.php?option=com_urlpolls&view=polls', $submenu === 'polls');
		}
		if ($user->authorise('recipient.access', 'com_urlpolls') && $user->authorise('recipient.submenu', 'com_urlpolls'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_URLPOLLS_SUBMENU_RECIPIENTS'), 'index.php?option=com_urlpolls&view=recipients', $submenu === 'recipients');
		}
	}
	...
	*/
	 ...
	 *
	 */
	public static function getVar($table, $where = null, $whereString = 'user', $what = 'id', $operator = '=', $main = 'urlpolls')
	{
		if(!$where)
		{
	 ...
	 *
	 */
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'urlpolls', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__urlpolls_'.$type.' AS a');
	 ...
	 * 
	 */
	public static function getActions($view, &$record = null, $views = null, $target = null, $component = 'urlpolls', $user = 'null')
	{
	 ...
	 */
	public static function getModel($name, $path = JPATH_COMPONENT_ADMINISTRATOR, $Component = 'Urlpolls', $config = array())
	{
		// fix the name
	...
	public static function setAsset($id, $table, $inherit = true)
	{
		...
		$parent->loadByName('com_urlpolls');
		
		$parentId = $parent->id;
		$name     = 'com_urlpolls.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_urlpolls', $table, $inherit);
			...
			else
			{
				...
				return JFactory::getDbo()->updateObject('#__urlpolls_'.$table, $object, 'id');
			}
		}
		return false;
	}
	...
	public static function transliterate($string)
	{
		...
		{
			// get global value
			self::$langTag = JComponentHelper::getParams('com_urlpolls')->get('language', 'en-GB');
		}
