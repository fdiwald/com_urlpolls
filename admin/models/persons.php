 ...
 * Persons Model
 */
class UrlpollsModelPersons extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$personname = $this->getUserStateFromRequest($this->context . '.filter.personname', 'filter_personname');
		if ($formSubmited)
		{
			$personname = $app->input->post->get('personname');
			$this->setState('filter.personname', $personname);
		}
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.personname LIKE '.$search.')');
			}
		}
	...
	*/
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.personname');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__urlpolls_person'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__urlpolls_person'))->set($fields)->where($conditions); 
