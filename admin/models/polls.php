 ...
 * Polls Model
 */
class UrlpollsModelPolls extends JModelList
{
...
/***[JCBGUI.admin_view.php_model_list.111.$$$$]***/
///***[/JCBGUI$$$$]***/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pollname = $this->getUserStateFromRequest($this->context . '.filter.pollname', 'filter_pollname');
		if ($formSubmited)
		{
			$pollname = $app->input->post->get('pollname');
			$this->setState('filter.pollname', $pollname);
		}
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__urlpolls_poll', 'a'));
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.pollname LIKE '.$search.')');
			}
		}
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__urlpolls_poll");
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.pollname');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__urlpolls_poll'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__urlpolls_poll'))->set($fields)->where($conditions); 
