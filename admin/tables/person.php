 ...
 * Persons Table class
 */
class UrlpollsTablePerson extends JTable
{
	/**
	...
	{
		parent::__construct('#__urlpolls_person', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_urlpolls.person'));
	}	
	...
	/**
	 * Overload the store method for the Person table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('person', 'UrlpollsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_URLPOLLS_PERSON_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_urlpolls.person.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_urlpolls.person.'.(int) $this->$k;
	}
