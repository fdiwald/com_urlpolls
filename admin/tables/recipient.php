 ...
 */
class UrlpollsTableRecipient extends JTable
{
	/**
	...
	{
		parent::__construct('#__urlpolls_recipient', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_urlpolls.recipient'));
	}	
	...
	/**
	 * Overload the store method for the Recipient table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New recipient. A recipient created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('recipient', 'UrlpollsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_URLPOLLS_RECIPIENT_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_urlpolls.recipient.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_urlpolls.recipient.'.(int) $this->$k;
	}
