 ...
 * Person View class
 */
class UrlpollsViewPerson extends JViewLegacy
{
	/**
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_URLPOLLS_PERSON_NEW' : 'COM_URLPOLLS_PERSON_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				$canVersion = ($this->canDo->get('core.version') && $this->canDo->get('person.version'));
				if ($this->state->params->get('save_history', 1) && $this->canDo->get('core.edit') && $canVersion)
				{
					JToolbarHelper::versions('com_urlpolls.person', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
		...
		$help_url = UrlpollsHelper::getHelpUrl('person');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_URLPOLLS_PERSON_NEW' : 'COM_URLPOLLS_PERSON_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_urlpolls/assets/css/person.css", (UrlpollsHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_urlpolls/views/person/submitbutton.js", (UrlpollsHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
