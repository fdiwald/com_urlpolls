...
/**
 * Urlpolls Route Helper
 **/
abstract class UrlpollsHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Answer
	 */
	public static function getAnswerRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			...
			// Create the link
			$link = 'index.php?option=com_urlpolls&view=answer&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_urlpolls&view=answer';
		}
		if ($catid > 1)
		{
			$categories = JCategories::getInstance('urlpolls.answer');
	...
	/**
	 * Get the URL route for urlpolls category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_urlpolls&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_urlpolls');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (UrlpollsHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
