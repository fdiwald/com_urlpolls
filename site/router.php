...
/**
 * Routing class from com_urlpolls
 *
 * @since  3.3
 */
class UrlpollsRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_urlpolls component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'answer'))
		{
	...
	{		
		...
		switch($segments[0])
		{
			case 'answer':
				...
				{
					$id = $this->getVar('person', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'urlpolls')
	{
...
function UrlpollsBuildRoute(&$query)
{
	$router = new UrlpollsRouter;
	
	return $router->build($query);
}

function UrlpollsParseRoute($segments)
{
	$router = new UrlpollsRouter;
